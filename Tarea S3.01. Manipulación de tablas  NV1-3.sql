...
SELECT *
FROM credit_card
WHERE id = "CcU-2938"; #Esta consulta carece de sentido tras el cambio pues una ves realizado ya muestra la informacion actualizada pero se realizo previamente a fin de verificar la informacion suministrada
...
SELECT c.company_name AS Nombre_de_la_Empresa, c.phone AS Telefono_de_contacto, c.country AS País_de_residencia, AVG(t.amount) AS Media_T_realizadas
FROM company c
JOIN transaction t ON t.company_id = c.id
WHERE t.declined in ("0")
GROUP BY Nombre_de_la_Empresa, Telefono_de_contacto, País_de_residencia
ORDER BY Media_T_realizadas;
...
WHERE v.País_de_residencia IN ("Germany");

/*Tarea S3.01. Manipulación de tablas
Nivel 3 
Ejercicio 1*/

#Se elimina el campo "website" de la tabla "company"
ALTER TABLE company 
DROP COLUMN website;

#Se crea el campo "fecha_actual" en la tabla "credit_card"
ALTER TABLE credit_card
ADD fecha_actual DATE;

#Se crea la nueva tabla "user"
...
#Se deshabilita el check de FK
SET foreign_key_checks = 0;

#Se crea la nueva FK de la tabla transaction
ALTER TABLE transaction
...
SELECT t.id AS ID_Transacción, u.name AS Nombre_de_Usuario, u.surname AS Apellido,  cc.iban AS IBAN, c.company_name AS Nombre_de_la_empresa, t.Declined, t.timestamp AS Fecha
FROM company c
